...
@Component({
  ...
  standalone: true,
  imports: [FontAwesomeModule, CommonModule, MatTableModule],
...
export class SolicitacoesAjustesComponent {
  ...
  selectedIndentificacao = 'Todos';
  indicatorPosition = '0%';


  selectStatus(status: string, modelo: number) {
    if (modelo == 1) {
      ...
      this.selectedIndentificacao = status;
      switch (status) {
        case 'Todos':
          this.indicatorPosition = '0%';
          break;
